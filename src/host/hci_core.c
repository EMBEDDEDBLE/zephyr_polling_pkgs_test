...
extern uint8_t hci_boot_process(uint8_t *hci_cmd_buffer, uint16_t *tx_len);

struct bt_dev_set bt_dev;
...
void bt_hci_host_num_completed_packets(struct net_buf *buf)
{
    ...
    if (!conn)
    {
        BT_WARN("Unable to look up conn with index 0x%02x", acl(buf)->index);
        return;
    }
...
static void hci_acl(struct net_buf *buf)
{
    ...
    if (!conn)
    {
        BT_ERR("Unable to find conn for handle %u", bt_acl_handle(handle));
        net_buf_unref(buf);
        return;
    }
...
static struct bt_conn *find_pending_connect(uint8_t role, bt_addr_le_t *peer_addr)
{
    ...
    {
        conn = bt_conn_lookup_state_le(BT_ID_DEFAULT, peer_addr, BT_CONN_CONNECTING);
    ...
    {
        conn = bt_conn_lookup_state_le(bt_dev.adv_conn_id, peer_addr, BT_CONN_CONNECTING_DIR_ADV);
        if (!conn)
        {
...
{
#if defined(CONFIG_BT_USER_PHY_UPDATE)
    if (IS_ENABLED(CONFIG_BT_AUTO_PHY_UPDATE) && IS_ENABLED(CONFIG_BT_EXT_ADV) &&
        BT_DEV_FEAT_LE_EXT_ADV(bt_dev.le.features))
    {
        if (conn->le.phy.tx_phy == BT_HCI_LE_PHY_2M && conn->le.phy.rx_phy == BT_HCI_LE_PHY_2M)
        {
            return true;
        }
    }
...
static void conn_auto_initiate(struct bt_conn *conn)
{
    ...
    if (!atomic_test_bit(conn->flags, BT_CONN_AUTO_FEATURE_EXCH) &&
        ((conn->role == BT_HCI_ROLE_CENTRAL) || BT_FEAT_LE_PER_INIT_FEAT_XCHG(bt_dev.le.features)))
    {
...
void bt_hci_le_enh_conn_complete(struct bt_hci_evt_le_enh_conn_complete *evt)
{
    ...
    BT_DBG("status 0x%02x handle %u role %u peer %s peer RPA %s", evt->status, handle, evt->role,
           bt_addr_le_str(&evt->peer_addr), bt_addr_str(&evt->peer_rpa));
    ...
    if (evt->status)
    {
        if (IS_ENABLED(CONFIG_BT_PERIPHERAL) && evt->status == BT_HCI_ERR_ADV_TIMEOUT)
        {
            le_conn_complete_adv_timeout();
            return;
        }

        if (IS_ENABLED(CONFIG_BT_CENTRAL) && evt->status == BT_HCI_ERR_UNKNOWN_CONN_ID)
        {
    ...
    if (evt->peer_addr.type == BT_ADDR_LE_PUBLIC_ID || evt->peer_addr.type == BT_ADDR_LE_RANDOM_ID)
    {
    ...
    {
        uint8_t id = evt->role == BT_HCI_ROLE_PERIPHERAL ? bt_dev.adv_conn_id : BT_ID_DEFAULT;
    ...
    if (!conn)
    {
        BT_ERR("No pending conn for peer %s", bt_addr_le_str(&evt->peer_addr));
...
static void le_data_len_change(struct net_buf *buf)
{
    ...
    BT_DBG("max. tx: %u (%uus), max. rx: %u (%uus)", max_tx_octets, max_tx_time, max_rx_octets,
           max_rx_time);
...
static int hci_send_cmd_le_set_event_mask(void)
{
    ...
    if (IS_ENABLED(CONFIG_BT_CONN))
    {
        ...
        if (IS_ENABLED(CONFIG_BT_PHY_UPDATE) &&
            (BT_FEAT_LE_PHY_2M(bt_dev.le.features) || BT_FEAT_LE_PHY_CODED(bt_dev.le.features)))
        {
...
static int hci_send_cmd_le_write_le_host_supp(void)
{
    struct net_buf *buf;
...
static void hci_cmd_done(uint16_t opcode, uint8_t status, struct net_buf *buf)
{
    ...
    {
        BT_WARN("opcode 0x%04x pool id %u pool %p != &hci_cmd_pool %p", opcode, buf->pool_id,
                net_buf_pool_get(buf->pool_id), &hci_cmd_pool);
        return;
    }
...
static void hci_notify_rx_event(struct bt_hci_evt_hdr *hdr, struct net_buf *buf)
{
    if (bt_dev.hci_state == HCI_STATE_BOOTING_WAIT ||
        bt_dev.hci_state == HCI_STATE_PREPARING_WAIT_CHIPSET)
    {
        if (bt_dev.chipset_drv && bt_dev.chipset_drv->event_process)
        {
            bt_dev.chipset_drv->event_process(hdr->evt, buf);
        }
    }

    if (bt_dev.hci_event_process)
    {
        bt_dev.hci_event_process(hdr->evt, buf);
    }
}
...
static void read_bd_addr_complete(struct net_buf *buf)
{
    ...
    BT_DBG("type: %d, addr: %s", addr.type, bt_addr_str_real(&addr.a));
...
{
    struct bt_hci_rp_le_read_wl_size *rp = buf->data;
    BT_DBG("status 0x%02x", rp->status);
    memcpy(bt_dev.le.wl_size, rp->wl_size, sizeof(bt_dev.le.wl_size));
#if defined(APP_CONFIG_BT_SLAVE_WHITELIST)
    extern uint8_t gWhiteListCmd;
    gWhiteListCmd = LE_ADD_DEV_TO_WL;
#endif
}
static void hci_handle_cmd_cmp_evt_le_add_dev_to_wl(struct net_buf *buf)
{
}
static void hci_handle_cmd_cmp_evt_le_clear_wl(struct net_buf *buf)
{
}
static void hci_handle_cmd_cmp_evt_le_rem_dev_to_wl(struct net_buf *buf)
{
}
#endif
...
#define HCI_COMMAND_COMPLETE_HANDLER(_opcode, _handler)                                            \
    {                                                                                              \
        .opcode = _opcode, .handler = _handler,                                                    \
    }

static const struct hci_command_complete_process_handler hci_cmd_cmp_handles[] = {
        HCI_COMMAND_COMPLETE_HANDLER(BT_HCI_OP_RESET, hci_reset_complete),
        HCI_COMMAND_COMPLETE_HANDLER(BT_HCI_OP_READ_LOCAL_FEATURES, read_local_features_complete),
        HCI_COMMAND_COMPLETE_HANDLER(BT_HCI_OP_READ_LOCAL_VERSION_INFO, read_local_ver_complete),
        ...
        HCI_COMMAND_COMPLETE_HANDLER(BT_HCI_OP_LE_ADD_DEV_TO_WL,
                                     hci_handle_cmd_cmp_evt_le_add_dev_to_wl),
        HCI_COMMAND_COMPLETE_HANDLER(BT_HCI_OP_LE_CLEAR_WL, hci_handle_cmd_cmp_evt_le_clear_wl),
        HCI_COMMAND_COMPLETE_HANDLER(BT_HCI_OP_LE_REM_DEV_FROM_WL,
                                     hci_handle_cmd_cmp_evt_le_rem_dev_to_wl),
#endif
};

static inline void
handle_hci_command_complete_work(uint16_t opcode, struct net_buf *buf,
...
static void hci_state_event_process(uint16_t opcode)
{
    ...
    BT_DBG("hci_state_event_process, opcode 0x%04x, hci_state: 0x%x, hci_init_state: 0x%x", opcode,
           bt_dev.hci_state, bt_dev.hci_init_state);

    if (bt_dev.hci_state == HCI_STATE_PREPARING_WAIT_RESET)
    {
    ...
    else if (bt_dev.hci_state == HCI_STATE_INITIALING_WAIT)
    {
        switch (bt_dev.hci_init_state)
        {
        case HCI_INIT_READ_LOCAL_FEATURES:
            if (opcode != BT_HCI_OP_READ_LOCAL_FEATURES)
            {
                return;
            }
            bt_dev.hci_init_state = HCI_INIT_READ_LOCAL_VERSION_INFO;
            bt_hci_cmd_send(BT_HCI_OP_READ_LOCAL_VERSION_INFO, NULL);
            break;
        case HCI_INIT_READ_LOCAL_VERSION_INFO:
            if (opcode != BT_HCI_OP_READ_LOCAL_VERSION_INFO)
            {
                return;
            }
            /* For now we only support LE capable controllers */
            bt_dev.hci_init_state = HCI_INIT_READ_SUPPORTED_COMMANDS;
            bt_hci_cmd_send(BT_HCI_OP_READ_SUPPORTED_COMMANDS, NULL);
            break;
        case HCI_INIT_READ_SUPPORTED_COMMANDS:
            if (opcode != BT_HCI_OP_READ_SUPPORTED_COMMANDS)
            {
                return;
            }
            /* For now we only support LE capable controllers */
...
#if defined(CONFIG_BT_CONN)
            bt_dev.hci_init_state = HCI_INIT_LE_READ_BUFFER_SIZE;
            /* Read LE Buffer Size */
            bt_hci_cmd_send(BT_HCI_OP_LE_READ_BUFFER_SIZE, NULL);
#else
            if (BT_FEAT_BREDR(bt_dev.features))
            {
                bt_dev.hci_init_state = HCI_INIT_LE_WRITE_LE_HOST_SUPP;
                hci_send_cmd_le_write_le_host_supp();
            }
            else
            {
                bt_dev.hci_init_state = HCI_INIT_LE_SET_EVENT_MASK;
        ...
        case HCI_INIT_LE_READ_BUFFER_SIZE:
            if (opcode != BT_HCI_OP_LE_READ_BUFFER_SIZE)
            {
                return;
            }
            if (BT_FEAT_BREDR(bt_dev.features))
            ...
            break;
#endif
        case HCI_INIT_LE_WRITE_LE_HOST_SUPP:
            if (opcode != BT_HCI_OP_LE_WRITE_LE_HOST_SUPP)
            {
                return;
            }
            bt_dev.hci_init_state = HCI_INIT_LE_SET_EVENT_MASK;
        ...
        case HCI_INIT_LE_SET_EVENT_MASK:
            ...
            if (BT_FEAT_BREDR(bt_dev.features))
            {
                bt_dev.hci_init_state = HCI_INIT_BREDR_READ_BUFFER_SIZE;
                bt_hci_cmd_send(BT_HCI_OP_READ_BUFFER_SIZE, NULL);
            }
            else
#endif
            {
                bt_dev.hci_init_state = HCI_INIT_SET_EVENT_MASK;
                hci_send_cmd_set_event_mask();
            }
            break;

#if defined(CONFIG_BT_CONN)
        case HCI_INIT_BREDR_READ_BUFFER_SIZE:
            if (opcode != BT_HCI_OP_READ_BUFFER_SIZE)
            {
                return;
            }
            bt_dev.hci_init_state = HCI_INIT_SET_EVENT_MASK;
        ...
        case HCI_INIT_READ_BD_ADDR:
            ...
            break;
        default:
            BT_WARN("hci_state_event_process, Unknown opcode: 0x%04x", opcode);
            break;
        }
    }
}
...
static void hci_cmd_complete(struct net_buf *buf)
{
    ...
    hci_state_event_process(opcode);
...
int hci_send_cmd_le_add_dev_to_wl()
{
    struct bt_hci_cp_le_add_dev_to_wl *cp_le_add_addr;
    struct net_buf *buf;
    bt_addr_le_t *add_wl_addr = &bt_dev.le.add_wl_addr;
    add_wl_addr->type = BT_ADDR_LE_PUBLIC;
    add_wl_addr->a.val[0] = (BLE_WL_TEST_ADDR)&0xff;
    add_wl_addr->a.val[1] = (BLE_WL_TEST_ADDR >> 8) & 0xff;
    add_wl_addr->a.val[2] = (BLE_WL_TEST_ADDR >> 16) & 0xff;
    add_wl_addr->a.val[3] = (BLE_WL_TEST_ADDR >> 24) & 0xff;
    add_wl_addr->a.val[4] = (BLE_WL_TEST_ADDR >> 32) & 0xff;
    add_wl_addr->a.val[5] = (BLE_WL_TEST_ADDR >> 40) & 0xff;
    /* Set LE event mask */
    buf = bt_hci_cmd_create(BT_HCI_OP_LE_ADD_DEV_TO_WL, sizeof(*cp_le_add_addr));
    if (!buf)
    {
        return -ENOBUFS;
    }
    cp_le_add_addr = net_buf_add(buf, sizeof(*cp_le_add_addr));
    memcpy(&cp_le_add_addr->addr, add_wl_addr, sizeof(*cp_le_add_addr));
    return bt_hci_cmd_send(BT_HCI_OP_LE_ADD_DEV_TO_WL, buf);
}
#endif

void hci_tx_cmd_packet(uint8_t *data, uint16_t len)
{
    struct net_buf *buf = bt_buf_get_host_tx_cmd();
    if (buf)
    {
    ...
    {
        BT_INFO("no reserve buff");
        while (1)
            ;
    }

    BT_INFO("data: %s", bt_hex(data, len));
    net_buf_put(&bt_dev.cmd_tx_queue, buf);
}
...
static void bt_dev_show_info(void)
{
    ...
    BT_INFO("Identity%s: %s", bt_dev.id_count > 1 ? "[0]" : "", bt_addr_le_str(&bt_dev.id_addr[0]));
    ...
    if (IS_ENABLED(CONFIG_BT_LOG_SNIFFER_INFO))
    {
        ...
        BT_INFO("IRK%s: 0x%s", bt_dev.id_count > 1 ? "[0]" : "", bt_hex(irk, 16));
#endif
    }

    for (i = 1; i < bt_dev.id_count; i++)
    {
        BT_INFO("Identity[%d]: %s", i, bt_addr_le_str(&bt_dev.id_addr[i]));
    ...
    BT_INFO("HCI: version %s (0x%02x) revision 0x%04x, manufacturer 0x%04x",
            ...
            bt_dev.manufacturer);
    BT_INFO("LMP: version %s (0x%02x) subver 0x%04x", ver_str(bt_dev.lmp_version),
...
{
    static const char *const plat_str[] = {"reserved", "Intel Corporation", "Nordic Semiconductor",
                                           "NXP Semiconductors"};
...
static void hci_vs_init(void)
{
    ...
    BT_INFO("HW Platform: %s (0x%04x)", vs_hw_platform(sys_le16_to_cpu(rp.info->hw_platform)),
    ...
    BT_INFO("HW Variant: %s (0x%04x)",
            ...
            sys_le16_to_cpu(rp.info->hw_variant));
    BT_INFO("Firmware: %s (0x%02x) Version %u.%u Build %u", vs_fw_variant(rp.info->fw_variant),
...
static int hci_init(void)
{
    ...
    sys_slist_init(&bt_dev.rx_queue);

    bt_dev.hci_state = HCI_STATE_BOOTING;
...
{
    BT_DBG("bt_send buf %p len %u type %u", buf, buf->len, bt_buf_get_type(buf));

    BT_PACKET_DUMP(bt_get_h4_type_by_buffer(bt_buf_get_type(buf)), 0, buf->data, buf->len);
...
int bt_recv(struct net_buf *buf)
{
    ...
    BT_PACKET_DUMP(bt_get_h4_type_by_buffer(bt_buf_get_type(buf)), 1, buf->data, buf->len);

    BT_DBG("rx: data: %p, len: %p", buf->data, buf->len);
    if (bt_buf_get_type(buf) == BT_BUF_EVT)
    {
        struct bt_hci_evt_hdr *hdr = (void *)buf->data;

        BT_DBG("evt: 0x%x, buf %p len %u pool %p", hdr->evt, buf, buf->len, buf->pool_id);
    }
    else
    {
        struct bt_hci_acl_hdr *hdr = (void *)buf->data;

        BT_DBG("handle: 0x%x, buf %p len %u pool %p", hdr->handle, buf, buf->len, buf->pool_id);
    }
...
#if defined(CONFIG_BT_CONN)
    // Be-careful, this will make hci transaction lock.
    if (bt_get_rx_acl_lock())
    {
        buf = (struct net_buf *)sys_slist_peek_head(&bt_dev.rx_queue);
        if (!buf)
        {
            return;
        }
        ...
        {
            BT_WARN("rx_acl_process_loop(), rx acl locked.\n");
            return;
        }
    }
#endif
...
void hci_state_polling(void)
{
    switch (bt_dev.hci_state)
    {
    case HCI_STATE_BOOTING:
        if (bt_dev.chipset_drv && bt_dev.chipset_drv->boot_start)
        {
            bt_dev.hci_state = HCI_STATE_BOOTING_WAIT;
            bt_dev.chipset_drv->boot_start();
            return;
        }
        bt_dev.hci_state = HCI_STATE_PREPARING;
        // just do it.
        // break;
    case HCI_STATE_PREPARING:
        bt_dev.hci_state = HCI_STATE_PREPARING_WAIT_RESET;
    ...
    case HCI_STATE_INITIALING:
        ...
        bt_dev.hci_init_state = HCI_INIT_READ_LOCAL_FEATURES;
...
int bt_enable(bt_ready_cb_t cb)
{
    ...
    // memset(disconnected_handles, 0x00, sizeof(uint16_t) * CONFIG_BT_MAX_CONN);
    bt_set_rx_acl_lock(false);

#if defined(CONFIG_BT_PRIVACY)
    bt_dev.rpa_timeout = CONFIG_BT_RPA_TIMEOUT,
#endif
...
int bt_storage_kv_get_appearance(void)
{
    ...
    {
        BT_DBG("Appearance set to %d", bt_dev.appearance);
    }
...
void register_event_handle(bt_hci_event_process_t handle)
{
    bt_dev.hci_event_process = handle;
}
...
{
    if (bt_dev.hci_state == HCI_STATE_PREPARING_WAIT_CHIPSET)
    {
        bt_dev.hci_state = HCI_STATE_INITIALING;
    }
}
...
static uint8_t bt_monitor_sleep_flag;
static uint8_t bt_monitor_sleep_lock_flag;
static k_ticks_t bt_monitor_sleep_next_timeout;

void bt_init_monitor_sleep(void)
{
    bt_monitor_sleep_flag = 0;
    bt_monitor_sleep_lock_flag = 0;
}

uint8_t bt_check_is_in_sleep(void)
{
    return bt_monitor_sleep_flag;
}

uint8_t bt_check_allow_sleep(void)
{
    if (bt_buf_check_allow_sleep()
#if defined(CONFIG_BT_CONN)
        && bt_conn_check_allow_sleep()
#endif
        && !bt_monitor_sleep_lock_flag)
    {
        return 1;
    }
    return 0;
...
{
    extern void bt_buf_monitor_sleep(void);
    bt_buf_monitor_sleep();

    extern void bt_conn_monitor_sleep(void);
    bt_conn_monitor_sleep();
}
...
void bt_sleep_prepare_work(void)
{
    ...
    // get recent timeout.
    bt_monitor_sleep_next_timeout = z_get_recent_timeout_expiry();
    bt_monitor_sleep_flag = 1;
    BT_INFO("end");
}

// For muti thread need this.
void bt_sleep_wakeup_work_start(void)
{
    bt_monitor_sleep_lock_flag = 1;
    BT_DBG("flag: %d", bt_monitor_sleep_flag);
    if (!bt_monitor_sleep_flag)
    {
        return;
    }
    BT_INFO("start");
    bt_buf_pool_init();
    bt_conn_sleep_wake_init();
    BT_INFO("end");
}
...
void bt_sleep_wakeup_with_timeout(void)
{
    ...
    // get recent timeout.
    // add some pre-up time
    if (sys_clock_tick_get() > bt_monitor_sleep_next_timeout + K_MSEC(5).ticks)
    {
        bt_sleep_wakeup_work();
    }
}
#endif

// think muti link work?
static uint8_t rx_acl_lock;
